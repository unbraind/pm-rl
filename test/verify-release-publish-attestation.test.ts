...
test("an unset later on the same line does not retroactively unbind an earlier use", () => {
  // Deliberate, and the safe half of an asymmetric choice. Resolution is per
  // line, so a line holding both a use and a later `unset` of the same name is
  // read against the state after the unset. `npm publish $FLAG; unset FLAG` is
  // therefore reported as unattested although the shell does pass the flag --
  // a false alarm, which fails a release rather than shipping one unverified.
  //
  // Snapshotting before the line's unsets instead would invert the error into a
  // bypass: `unset FLAG; npm publish $FLAG` would then borrow a flag the shell
  // has already discarded. Only segment-level resolution decides both correctly,
  // and no workflow in this fleet writes either shape -- `unset` appears in
  // these files only inside comments.
  const useThenUnset = auditPublishAttestation([{
    file: "release.yml",
    text: ["FLAG=--provenance", "npm publish --access public $FLAG; unset FLAG"].join("\n"),
  }]);
  assert.equal(useThenUnset.failures.length, 1, "the conservative direction: a false alarm, not a pass");
